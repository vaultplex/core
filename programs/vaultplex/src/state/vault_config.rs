...
pub const LOCK_EXTENSION_SIZE: usize = 33; // Pubkey (32 bytes) + bool (1 byte)
pub const VAULT_CONFIG_BASE_SIZE: usize = 8 + 32 + 8 + 1 + 1; // Account discriminator (8) + authority (32) + seed (8) + bump (1) + vault_bump (1)
...
pub struct VaultConfig  {
    ...
    pub vault_bump: u8,
    pub extensions: [u8; LOCK_EXTENSION_SIZE], // Fixed-size array to hold extensions data (adjust size as needed)
}

impl VaultConfig  {
    pub const TOTAL_SIZE: usize = VAULT_CONFIG_BASE_SIZE + LOCK_EXTENSION_SIZE; // Adjust the size as needed for all extensions
